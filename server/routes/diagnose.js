...
const SYSTEM_PROMPT = `You are a 3D printing quality expert specializing in FDM printers like the QIDI X-Plus 3. Analyze the provided image of a 3D print (in progress or completed) and identify any print quality issues.
...
async function runDiagnosis({ imageBuffer, imagePath, printFile, autoTriggered, config }) {
  ...
  const body = {
    ...
    messages: [
      { role: 'system', content: SYSTEM_PROMPT },
      {
        role: 'user',
        content: [
          { type: 'text', text: 'Analyze this 3D print image and identify any quality issues.' },
  ...
  return { ...record, issues: parsed.issues, summary: parsed.summary };
}
...
router.post('/', upload.single('image'), async (req, res) => {
  ...
  if (req.file) {
    ...
    // Take live snapshot from printer
